...
tester <- function(counts, levels, methods = c("tree", "ldag", "part"), regular = F) {
  ...
  for (method in methods) {
    res <- ldags:::optimize_partition(counts, levels, ess = 1, method, regular = regular)
    df[[method]] <- get_parts(res$partition)
    scores[method] <- sum(res$scores)
  }
  df[["none"]] <- seq_len(nrow(counts))
  scores["none"] <- sum(compute_local_bdeu_score(counts, 1, ncol(counts), nrow(counts), s = 1))
...
```{r}
cat("Score, P(Y|X1, X2):")
sum(compute_local_bdeu_score(counts, ess = 1, r = 2, q = 4, s = 1))
cat("Score, P(Y|X2):")
sum(compute_local_bdeu_score(rowsum(counts, c(1, 1, 2, 2)), ess = 1, r = 2, q = 4, s = c(2, 2)))
cat("Score P(Y):")
compute_local_bdeu_score(matrix(colSums(counts), 1), ess = 1, r = 2, q = 4, s = 4)
```
