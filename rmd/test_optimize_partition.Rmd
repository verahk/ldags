...
ldags:::score_from_lookup(data, levels, lengths(levels), j, parentnodes, ess, method = "tree", lookup = tab)
ls.str(tab)
```
...
j <- 7
levels <- rep(list(0:1), 10)
nlev <- lengths(levels)
parentnodes <- which(bnlearn::amat(bn)[, j] == 1)

samplesizes <- c(100, 300, 1000, 3000, 10000)
scores <- matrix(nrow = length(samplesizes), ncol = 3)
colnames(scores) <- c("tree", "ldag", "dag")
for (i in seq_along(samplesizes)) {
  data <- bida:::sample_data_from_bn(bn, samplesizes[i])
  scores[i, 1] <- compute_local_bdeu_score_from_data(data, levels, nlev, j, parentnodes, ess = 1, "tree", NULL)
  scores[i, 2]  <- compute_local_bdeu_score_from_data(data, levels, nlev, j, parentnodes, ess = 1, "ldag", NULL)
  scores[i, 3]  <- compute_local_bdeu_score_from_data(data, levels, nlev, j, parentnodes, ess = 1, NULL, NULL)
}

scores

data <- bida:::sample_data_from_bn(bn, 300)
counts <- compute_freq_table(data, nlev, j, parentnodes)
tree <- optimize_partition(counts, levels[parentnodes], 1, "tree")
ldag <- optimize_partition(counts, levels[parentnodes], 1, "ldag")

cpt <- round(matrix(aperm(bn[[7]]$prob, paste0("X", c(parentnodes, j))), ncol = 2), 2)

cbind(expand.grid(levels[parentnodes]), 
      cpt, 
      tree = unlist_partition(tree$partition),
      ldag = unlist_partition(ldag$partition))
```
