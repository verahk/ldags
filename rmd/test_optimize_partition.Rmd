...
counts <- cbind(10, 10**rep(0:3, 2)) #seq(0, 3, length.out = 8))

for (reg in c(TRUE, FALSE)) {
  fits <- list(ldag = optimize_partition(counts, levels, 1, "ldag", regular = reg, verbose = T),
               tree = optimize_partition(counts, levels, 1, "tree", regular = reg, verbose = T))
  print(cbind(counts, sapply(fits, function(x) get_parts(x$partition))))
  print(sapply(fits, function(x) sum(x$scores)))
}

levels <- rep(list(0:1), 3)
counts <- cbind(10, 10*seq(0, 3, length.out = 8))
for (reg in c(TRUE, FALSE)) {
  fits <- list(ldag = optimize_partition(counts, levels, 1, "ldag", regular = reg, verbose = F),
               tree = optimize_partition(counts, levels, 1, "tree", regular = reg, verbose = F))
  print(cbind(counts, sapply(fits, function(x) get_parts(x$partition))))
  print(sapply(fits, function(x) sum(x$scores)))
}
