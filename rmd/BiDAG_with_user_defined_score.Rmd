...
sapply(files, source)
```
Define a Bayesian network over three nodes, a collider structure, where the CPT of the collider node $Y$ contains context-specific independence, i. e. a label on the one edge pointing into $Y$.
```{r}
dag <- rbind(Z = c(0, 0, 1),
             X = c(0, 0, 1),
             Y = c(0, 0, 0))
...
# Y is independent of Z given X = 0
p <- .75
